...
export const ReferralFormUpload: React.FC<ReferralFormUploadProps> = ({
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > 0) {
      ...
      // Add document type to the file
      const fileWithMetadata = newFile as File & { documentType?: string };
      fileWithMetadata.documentType = documentType;
      
      setSelectedFiles([fileWithMetadata]);
      onFilesSelected([fileWithMetadata]);
  ...
  // Component to display patient association status
  const renderPatientAssociationStatus = () => {
    if (!hasPatientAssociation) return null;
        ...
        <div className=" rounded-lg p-6">
          ...
          <div className="text-center flex flex-col gap-1">
            ...
            {scannedPatientInfo?.birthYear && (
              <span className='text-sm text-gray-400'> (Born {scannedPatientInfo.birthYear})</span>
            )}
            <p className="text-xs text-gray-600">
              {/* add time on the last scan */}
              Last scanned on {lastScannedDate ? new Date(lastScannedDate).toLocaleString() : 'First time scan'}
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center px-6 py-8">
      {/* Header Icon */}
      {!hasPatientAssociation && <div className="mb-6">
      ...
      {!hasPatientAssociation && <div className="text-center mb-8">
      ...
      {renderPatientAssociationStatus()}
        ...
        <div className="w-full max-w-md mb-8">
          ...
          >
            <div className="flex flex-col items-center">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${errors.length > 0 ? 'bg-red-100' : 'bg-gray-100'
                }`}>
        ...
        <div className="w-full max-w-md mb-8">
          <div className="text-center">
            ...
            <p className="text-xs text-gray-500 mt-2">
              {showUploadForm
                ? 'You can upload a new referral form to update patient information'
                : 'Click to upload a new referral form if needed'
              }
            </p>
          </div>
        </div>
            ...
            {previews.map((preview, index) => (
              <div key={preview.name + index} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg bg-gray-50">
                ...
                <button
                  ...
                  className={`ml-2 p-1 flex-shrink-0 ${isUploading
